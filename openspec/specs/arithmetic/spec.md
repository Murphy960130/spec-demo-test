...
## Purpose
arithmetic capability 提供基础算术运算函数。当前覆盖加法 `add(a, b)`，接收两个**同类型**入参（均为 `number` 或均为纯数字字符串，如 `'1'`、`'-3'`）返回两数之和（`number` 类型）；类型混用、非纯数字字符串（含小数点、空串等）及 `null`/`undefined` 等非法入参返回 `NaN`，不抛异常。后续按需扩展 sub/mul/div 等运算，均遵循纯函数 + 非法输入返回结果（NaN）的约定。
## Requirements
### Requirement: 加法运算
